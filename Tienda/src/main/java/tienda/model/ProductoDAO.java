...
public class ProductoDAO {
    ...
    // 7. Buscar productos activos por nombre o categoría (para la API de búsqueda)
    public List<Producto> buscarProductos(String query) {
        List<Producto> lista = new ArrayList<>();
        
        // Si el término viene vacío o nulo, retorna la lista vacía directamente
        if (query == null || query.trim().isEmpty()) {
            return lista;
        }

        String sql = "SELECT * FROM productos WHERE activo = TRUE AND " +
                     "(LOWER(nombre) LIKE LOWER(?) OR LOWER(categoria) LIKE LOWER(?)) " +
                     "ORDER BY id DESC";
